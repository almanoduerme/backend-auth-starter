...
import VerificationCodeModel from "../../databases/mongodb/verificationCode.model";
import appAssert from "../../utils/appAssert";
import { hashValue } from "../../libs/bcrypt/bcrypt";
import { ONE_DAY_MS, fiveMinutesAgo, oneHourFromNow, oneYearFromNow, thirtyDaysFromNow } from "../../utils/date";
import { getPasswordResetTemplate, getVerifyEmailTemplate } from "../../libs/resend/emailTemplates";
import { RefreshTokenPayload, refreshTokenSignOptions, signToken, verifyToken } from "../../libs/jwt/jwt";
import { sendMail } from "../../libs/resend/sendMail";
...
export const createAccount = async (data: CreateAccountParams) => {
  ...
  const verificationCode = await VerificationCodeModel.create({ userId, type: VerificationCodeType.EmailVerification, expiresAt: oneYearFromNow() });
...
export const loginUser = async ({ email, password, userAgent }: LoginParams) => {
  ...
  const sessionInfo: RefreshTokenPayload = { sessionId: session._id };
...
export const verifyEmail = async (code: string) => {
  const validCode = await VerificationCodeModel.findOne({ _id: code, type: VerificationCodeType.EmailVerification, expiresAt: { $gt: new Date() } });
...
export const sendPasswordResetEmail = async (email: string) => {
  ...
  try {
    ...
    const verificationCode = await VerificationCodeModel.create({ userId: user._id, type: VerificationCodeType.PasswordReset, expiresAt });

    const url = `${APP_ORIGIN}/password/reset?code=${verificationCode._id}&exp=${expiresAt.getTime()}`;

    const { data, error } = await sendMail({ to: email, ...getPasswordResetTemplate(url) });
